...
var _ = Describe("Batch", func() {
	...
	writeChangeFile := func(change core.Change) {
		...
		Expect(afs.WriteFile(filePath, bs, core.DefaultFileMode)).To(Succeed())
	}

	writeFutureFile := func(header, configPath string) {
		headerData := []byte(header)
		headerPath := filepath.Join(futurePath, configPath)
		Expect(afs.WriteFile(headerPath, headerData, core.DefaultFileMode)).To(Succeed())
	}
...
`
	...
	It("returns error on bad config", func() {
		...
		err := afs.WriteFile(core.ConfigPaths[0], configData, core.DefaultFileMode)
		Expect(err).To(BeNil())
	...
	It("returns error on bad changes", func() {
		...
		err := afs.WriteFile(filepath.Join(futurePath, "a.yaml"), aVer, core.DefaultFileMode)
		Expect(err).To(BeNil())
	...
	It("can get all changes", func() {
		...
		Expect(afs.WriteFile(ignoredPath, []byte("ignored"), core.DefaultFileMode)).To(Succeed())
	...
	It("returns err if bad changes file", func() {
		...
		err := afs.WriteFile(filepath.Join(futurePath, "a.yaml"), badYaml, core.DefaultFileMode)
		Expect(err).To(BeNil())
